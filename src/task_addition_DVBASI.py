...
def main(rank, args):
    ...
    for dataset in datasets:
        if args.finetuning_mode == "linear":
            pretrained_checkpoint = f"/data/atlas/checkpoints/checkpoints_reg_efficient_1/ViT-B-32/{dataset}/linear_zeroshot.pt"
            finetuned_checkpoint = f"/data/atlas/checkpoints/checkpoints_reg_efficient_1/ViT-B-32/{dataset}/linear_finetuned.pt"
    ...
    if args.finetuning_mode == "linear":
        with open(os.path.join(args.save, "ft_accuracies.json"), 'r') as f:
            ...
            pretrained_path = f' ' #input the path of pretrained weights 
            pretrained_params = torch.load(pretrained_path)
            optima_path = f' '  #input the path of last DV-BASI iteration model weights
    ...
    if linearized_finetuning:
        ...
        log_path = os.path.join(args.save, "linear_svhn_1.json")
        coef = ddp_model.module.image_encoder.model.coef
    else:
        head_path = os.path.join(ckpdir, "addition_baseline")
        log_path = os.path.join(args.save, "addition_baseline")
