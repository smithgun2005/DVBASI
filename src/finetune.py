...
def finetune(rank, args):
    ...
    if args.save is not None and is_main_process():
        zs_path = (
            os.path.join(ckpdir, "linear_zeroshot_3.pt")
            if linearized_finetuning
            else os.path.join(ckpdir, "zeroshot_3.pt")
        )
        ft_path = (
            os.path.join(ckpdir, "linear_finetuned_1.pt")
            if linearized_finetuning
            else os.path.join(ckpdir, "finetuned_1.pt")
        )
