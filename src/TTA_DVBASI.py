...
from src.utils import cosine_lr, get_preds, TIPWrapper, LPPWrapper, IndexWrapper, _RepeatSampler, TwoStreamBatchSampler, \
...
def ssl_loss_trusted(logits1: torch.Tensor, logits2: torch.Tensor, targets: torch.FloatTensor = None,
                     trusted: torch.BoolTensor = None, thresh: float = 0.99) -> torch.Tensor:

    one_hot = logits1.softmax(1).detach()

    guessed_targets = one_hot * (.5)
...
def main(rank, args):
    ...
    pool = ["Food101",
        "SUN397", "SVHN",
        "CIFAR10", "CIFAR100", "ImageNet", "STL10", "Caltech101", "Caltech256",
        "FGVCAircraft", "Flowers102", "OxfordIIITPet", "CUB200", "PascalVOC", "Country211", "UCF101", "Cars", "DTD",
        "EuroSAT", "GTSRB", "MNIST", "RESISC45"
    ]
    task_vectors = {}
    for dataset in pool:
...
def train(task_vectors, args, comp_acc={}):
    ...
    if is_main_process():
        if f"{target_dataset}_zeroshot" not in comp_acc.keys():
            comp_acc[f"{target_dataset}_zeroshot"] = \
                eval_single_dataset(image_encoder, target_dataset.replace('Val', ''), args)["top1"]
    ...
    for epoch in range(args.epochs):
        ...
        if is_main_process():
            ...
            coef = image_encoder.coef
            acc = eval_single_dataset(image_encoder, target_dataset, args)["top1"]
            print(f"Epoch [{epoch + 1}/{args.epochs}] - validation accuracy: {acc:.4f}")
    ...
    if is_main_process():
        ...
        best_params = image_encoder.get_optimized_params()
        heads[target_dataset] = best_params
...
def train_adapter(ddp_model, ddp_loader, args, comp_acc, which='lpp'):
    ...
    if which == 'lpp':
        if isinstance(args.subsample, float):
            shots = 100
        else:
    ...
    for i, batch in enumerate(ddp_loader):
        ...
        if (
            ...
            percent_complete = 100 * (i + 1) / len(ddp_loader)
            print(
                f"Train Epoch: {epoch} [{percent_complete:.0f}% {i + 1}/{len(ddp_loader)}]\t"
                f"Loss: {loss.item():.6f}\tData (t) {data_time:.3f}\tBatch (t) {batch_time:.3f}",
                flush=True,
            )
    ...
    if is_main_process():
        ...
        comp_acc[target_dataset + f"_{which}"] = \
            eval_single_dataset(image_encoder, target_dataset, args, model=ddp_model)["top1"]
        with open(args.log_path, 'w') as f:
            json.dump(comp_acc, f, indent=4)
...
if __name__ == "__main__":
    ...
    args.target_datasets = {k: 10 for k, v in args.target_datasets.items()}

    args.save = os.path.join('/data/atlas/checkpoints', f'{args.model}')
    if args.seed is not None:
        args.logdir += f"/{args.seed}"

    args.head_path = os.path.join(args.logdir, "")
    args.log_path = os.path.join(args.logdir, "")
